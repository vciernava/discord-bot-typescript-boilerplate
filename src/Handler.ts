...
export default class Handler {
  ...
  constructor(
    bot: ClientWithCommands,
    options: { commandsDir: string; eventsDir: string; featuresDir: string }
  ) {
    this.bot = bot;
  ...
  async loadCommands() {
    ...
    for (const file of files) {
      ...
      logger.info(`Loaded command: ${file.split('.')[0]}`);
    }
  }
  ...
  async loadEvents() {
    ...
    for (const file of files) {
      ...
      logger.info(`Loaded event: ${file.split('.')[0]}`);
    }
  }
  ...
  async loadFeatures() {
    ...
    for (const file of files) {
      ...
      logger.info(`Loaded feature: ${file.split('.')[0]}`);
    }
  }
}
